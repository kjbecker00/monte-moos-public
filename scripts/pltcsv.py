...
def plot_csv(input_file, figname, file_type, x_header, y_headers, plot_title):
    ...
    y_columns = [header.index(y_header) for y_header in y_headers if y_header in header]
    ...
    for i, y_column in enumerate(y_columns):
        y = data[:, y_column]
        plt.scatter(x, y, color=cmap.to_rgba(i), label=header[y_column], marker=marker, s=size)
...
def main():
    ...
    for (i, arg) in enumerate(sys.argv):
        ...
        if (arg.startswith("-x")):
            ...
            continue
        if (arg.startswith("-y")):
            y_plots = [[y.strip() for y in ys.split(",")] for ys in arg[len("-y="):].split(";")]
            continue
        if (arg.endswith(".csv")):
            input_file = arg
            continue
    ...
    if len(y_plots)!=1:
        assert len(x_cols)==len(y_plots), f"Error: Number of x variables must equal number of y plots (seperate x's with ',' and seperate y plots with ';')"
    if (len(x_cols)==1):
        plot_csv(input_file, figname, file_type, x_cols[0], y_plots[0], plot_title)
    else:
        for i, x_col in enumerate(x_cols):
            ...
            # print(f"{plot_title}: x={x_col} vs y={y_plots[i]}")
            plot_csv(input_file, fname, file_type, x_col, y_plots[i], plot_title)
