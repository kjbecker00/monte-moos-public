...
for item in "job_dirs"/*; do
    ...
    vecho "  encrypting job_dir with ./scripts/encrypt_file.sh job_dirs/$JOB_DIR" 3
    ./scripts/encrypt_file.sh "job_dirs/$JOB_DIR" >/dev/null
    EXIT_CODE=$?
    [ $EXIT_CODE -eq 0 ] || { vexit "running ./scripts/encrypt_file.sh returned exit code: $EXIT_CODE" 4; }
